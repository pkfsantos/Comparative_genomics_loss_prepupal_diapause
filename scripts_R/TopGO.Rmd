...
cDE_cont_genes <- read.table("relaxation_of_selection.txt",header = FALSE)
cDE_cont_genes <- as.charactercDE_cont_genes <- as.charactercDE_cont_genes <- as.character(cDE_cont_genes$V1)

geneList <- factor(as.integer(geneUniverse%in%cDE_cont_genes))
names(geneList) <- geneUniverse

myGOdata <- new("topGOdata",description="intensification_selection", ontology="BP",allGenes=geneList, annot = annFUN.gene2GO, gene2GO = geneID2GO)

resultFisher <- runTest(myGOdata, algorithm = "weight01", statistic="fisher")
resultFisher

allRes <- GenTable(myGOdata, raw.p.value = resultFisher, classicFisher = resultFisher, orderBy = "classicFisher", ranksOf = "classicFisher", topNodes = 40,numChar=1000)
allRes

#write.table(allRes, file = "relaxation_of_selection_BP.txt", sep = "\t", quote = FALSE, row.names = F)
```
Visualization
